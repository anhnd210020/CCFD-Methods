...
# Loading the data
file_path = '/home/ducanh/Credit Card Transactions Fraud Detection/Datasets/combined_data_with_deltat.csv'
...
# Không áp dụng Borderline-SMOTE - sử dụng dữ liệu gốc sau scale
...
cutoff_train_df = pd.DataFrame(columns=['Threshold', 'Accuracy', 'precision_score', 'recall_score', 'F1_score'])

for thresh in numbers:
    # Dự đoán nhị phân theo threshold
    y_pred_bin = y_train_results['pred_fraud'].map(lambda x: 1 if x > thresh else 0)
    cm = confusion_matrix(y_train_results['y_train_actual'], y_pred_bin)
    ...
    cutoff_train_df.loc[thresh] = [thresh, accuracy, precision, recall, f1_score_value]

print("\n--- Train Set Evaluation ---")
print(cutoff_train_df)

best_idx_train = cutoff_train_df['F1_score'].idxmax()
best_threshold_train = cutoff_train_df.loc[best_idx_train, 'Threshold']
best_auc_train = roc_auc_score(y_train_results['y_train_actual'], y_train_results['pred_fraud'])
print(f'Train - Best Threshold: {best_threshold_train:.4f}')
print(f'Train - Best ROC_AUC Score: {best_auc_train:.4f}')

# ------------------ Tính các chỉ số hiệu năng cho tập test ------------------
cutoff_test_df = pd.DataFrame(columns=['Threshold', 'Accuracy', 'precision_score', 'recall_score', 'F1_score'])

for thresh in numbers:
    y_pred_bin = y_test_results['pred_fraud'].map(lambda x: 1 if x > thresh else 0)
    cm = confusion_matrix(y_test_results['y_test_actual'], y_pred_bin)
    TP = cm[1, 1] if cm.shape[0] > 1 and cm.shape[1] > 1 else 0
    FP = cm[0, 1] if cm.shape[1] > 1 else 0
    FN = cm[1, 0] if cm.shape[0] > 1 else 0
    TN = cm[0, 0]
    ...
    cutoff_test_df.loc[thresh] = [thresh, accuracy, precision, recall, f1_score_value]

print("\n--- Test Set Evaluation ---")
print(cutoff_test_df)

best_idx_test = cutoff_test_df['F1_score'].idxmax()
best_threshold_test = cutoff_test_df.loc[best_idx_test, 'Threshold']
best_accuracy_test = cutoff_test_df.loc[best_idx_test, 'Accuracy']
best_precision_test = cutoff_test_df.loc[best_idx_test, 'precision_score']
best_recall_test = cutoff_test_df.loc[best_idx_test, 'recall_score']
best_f1_score_test = cutoff_test_df.loc[best_idx_test, 'F1_score']
best_auc_test = roc_auc_score(y_test_results['y_test_actual'], y_test_results['pred_fraud'])
print(f'Test - Best Threshold: {best_threshold_test:.4f}')
...
print(f'Test - Best F1 Score: {best_f1_score_test:.4f}')
print(f'Test - Best ROC_AUC Score: {best_auc_test:.4f}')

# ------------------ Vẽ ROC Curve cho tập test ------------------
fpr, tpr, _ = roc_curve(y_test_results['y_test_actual'], y_test_results['pred_fraud'])
plt.figure(figsize=(8, 6))
plt.plot(fpr, tpr, label=f'ROC Curve (AUC = {best_auc_test:.4f})')
