...
# ----------------------
# Áp dụng Borderline-SMOTE và huấn luyện KNN
# ----------------------
...
# KNN sử dụng dữ liệu đã oversample
knn_model = KNeighborsClassifier()  # you can adjust n_neighbors if needed
knn_model.fit(X_train_sc_bsmote, y_train_bsmote)

# Dự đoán xác suất trên tập huấn luyện và kiểm tra
y_pred_train_proba = knn_model.predict_proba(X_train_sc)
y_pred_test_proba = knn_model.predict_proba(X_test_sc)
...
# Vì KNN không có feature coefficients, chúng ta thông báo thay cho feature importance.
print("\nKNN không cung cấp feature importances trực tiếp.")

# Optionally, nếu bạn muốn hiển thị một biểu đồ thay cho feature importance thì bạn có thể chuyển sang các phương pháp khác.
# Ví dụ: sử dụng Permutation Importance (không bao gồm ở đây)
