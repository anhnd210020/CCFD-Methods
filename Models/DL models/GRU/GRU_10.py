...
class FraudGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        ...
        # Insert 
        #  before the GRU.
        # Here, we assume a singleton node dimension (num_nodes=1).
        self.shift_gcn = Shift_gcn(in_channels=input_size, 
                                   out_channels=hidden_size, 
                                   num_nodes=1)
        # GRU now takes the output dimension of Shift_gcn (hidden_size) as input.
    ...
    def forward(self, x):
        ...
        # Reshape to add a singleton node dimension: (batch, seq, 1, input_size)
        x = x.unsqueeze(2)
        # Apply Shift_gcn: expected output shape: (batch, hidden_size, seq, 1)
        x = self.shift_gcn(x)
        # Remove the singleton node dimension and permute to (batch, seq, hidden_size)
        x = x.squeeze(3).permute(0, 2, 1)
        # Pass through GRU
        out, _ = self.gru(x)
        # Use the hidden state from the last time step for prediction
        out = self.fc(out[:, -1, :])
