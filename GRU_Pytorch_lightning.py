...
# =============================================================================
class FraudGRU(L.LightningModule):
    def __init__(self, input_size, hidden_size, num_layers, criterion):
    ...
    def validate_step(self, batch, batch_idx):
        ...
        outputs = self(X_batch).squeeze()
        loss = self.criterion(outputs, y_batch)
        self.log("validate_loss", loss, on_step=True)
        return loss
...
model = FraudGRU(input_size, hidden_size, num_layers, criterion)

num_epochs = 30

trainer = L.Trainer(devices=1, max_epochs = 20, logger=True)
