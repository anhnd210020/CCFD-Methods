...
# Note: shuffle only shuffles the order of sequences, not the order within each sequence.
...
class QuantumLayer(nn.Module):
    def __init__(self, n_qubits):
        ...
        self.weights = nn.Parameter(torch.randn(n_qubits, dtype=torch.float32))
    
    def forward(self, x):
        # x có shape: (batch_size, n_qubits)
        outputs = []
        for sample in x:
            # Gọi qnode cho mỗi sample
            out = quantum_circuit(sample.float(), self.weights)
            outputs.append(torch.stack(out))
        return torch.stack(outputs)

    
class FraudGRU_Attention_Quantum(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_heads, n_qubits):
        super(FraudGRU_Attention_Quantum, self).__init__()
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True)
        self.attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True)
        self.layernorm = nn.LayerNorm(hidden_size)
        self.fc_classical = nn.Linear(hidden_size, n_qubits)
    ...
    def forward(self, x):
        ...
        gru_out, _ = self.gru(x)  # (batch_size, sequence_length, hidden_size)
        attn_out, _ = self.attention(gru_out, gru_out, gru_out)
        out = self.layernorm(gru_out + attn_out)
        pooled = out.mean(dim=1)  # Mean pooling theo sequence
        quantum_input = self.fc_classical(pooled)  # (batch_size, n_qubits)
        quantum_output = self.quantum_layer(quantum_input)  # (batch_size, n_qubits)
        fc_out = self.fc(quantum_output)  # (batch_size, 1)
...
model = FraudGRU_Attention_Quantum(input_size, hidden_size, num_layers, num_heads, n_qubits)
...
for i in numbers:
    ...
    # Giả sử confusion matrix có dạng: [[TN, FP], [FN, TP]]
    ...
    # Lưu theo đúng thứ tự: Threshold, Accuracy, precision, recall, F1
