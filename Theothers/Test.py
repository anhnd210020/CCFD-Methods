...
class THLSTMCell(nn.Module):
    ...
    def forward(self, x_t, delta_t, h_prev, c_prev):
        s_t = torch.tanh(self.W_sh(h_prev) + self.W_sx(x_t) + self.W_st(delta_t) + self.b_s) #(1)
        f_t = torch.sigmoid(self.W_fh(h_prev) + self.W_fx(x_t) + self.W_fs(s_t) + self.b_f) #(2)
        i_t = torch.sigmoid(self.W_ih(h_prev) + self.W_ix(x_t) + self.W_is(s_t) + self.b_i) #(3)
        T_t = torch.sigmoid(self.W_Th(h_prev) + self.W_Tx(x_t) + self.W_Ts(s_t) + self.b_T) #(4)
        zeta = torch.tanh(self.W_uh(h_prev) + self.W_ux(x_t) + self.W_us(s_t) + self.b_u) #(5)
        c_t = f_t * c_prev + i_t * zeta + T_t * s_t #(6)
        o_t = torch.sigmoid(self.W_oh(h_prev) + self.W_ox(x_t) + self.W_os(s_t) + self.b_o) #(7)
        h_tilde = o_t * torch.tanh(c_t) #(8)
        return h_tilde, c_t
...
class CurrentHistoricalAttention(nn.Module):
    ...
    def forward(self, h_tilde, c_t, h_history):
        q_t = torch.cat([h_tilde, c_t], dim=1) #(9)
        ...
        score_pre = torch.tanh(q_t_expanded + h_hist_proj + self.ba) #(10)
        alpha_unscaled = torch.einsum('bdh,h->bd', score_pre, self.v_t) 
        alpha = F.softmax(alpha_unscaled, dim=1)  # (B,d) #(11)
        alpha_expanded = alpha.unsqueeze(-1)
        e_t = (alpha_expanded * h_hist_stack).sum(dim=1) #(12)
        return e_t, alpha
...
# Tổng hợp TH-LSTM model
class THLSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, memory_size, num_classes=2):
    ...
    def forward(self, x_seq, delta_seq, g_seq):
        ...
        # g_seq: (B, seq_len, input_dim) => transactional expansion, ở đây dùng chính x_seq đã scale
        ...
        outputs = torch.cat(outputs, dim=1)  # (B, seq_len, num_classes)
        return outputs
# %%
...
# Giả sử train_sequences có shape (N, seq_len, num_features)
X_train_tensor = torch.tensor(X_train_seq, dtype=torch.float32)
X_test_tensor = torch.tensor(X_test_seq, dtype=torch.float32)
# Tạo delta_seq giả định: ví dụ, dùng giá trị ngẫu nhiên > 0, có shape (N, seq_len, 1)
...
# Ở đây g_seq = x_seq (có thể thay đổi nếu cần xử lý thêm)
...
test_dataset = TensorDataset(X_train_tensor, delta_train_tensor, g_train_tensor, y_train_tensor)
...
hidden_size = 64
num_layers = 2
model = THLSTMModel(input_dim=X_train_tensor.size(2), hidden_dim=64, memory_size=3, num_classes=2).to(device)
