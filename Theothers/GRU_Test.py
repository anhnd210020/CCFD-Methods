...
class FraudGRU_Attention(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_heads):
        super(FraudGRU_Attention, self).__init__()
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True)
        # Multi-head self-attention using GRU output as query, key, and value.
        self.attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True)
        # LayerNorm applied over the hidden dimension
    ...
    def forward(self, x):
        ...
        # Apply self-attention using GRU outputs as query, key, and value
        attn_out, _ = self.attention(gru_out, gru_out, gru_out)  # shape: (batch_size, sequence_length, hidden_size)
        # Residual connection and Layer Normalization
        out = self.layernorm(gru_out + attn_out)
        # Mean pooling over the sequence dimension
        pooled = out.mean(dim=1)  # shape: (batch_size, hidden_size)
...
input_size = X_train_seq.shape[2]  # Number of features (after removing cc_num)
hidden_size = 64
num_layers = 2
num_heads = 4  # Adjust number of attention heads as needed

model = FraudGRU_Attention(input_size, hidden_size, num_layers, num_heads)
