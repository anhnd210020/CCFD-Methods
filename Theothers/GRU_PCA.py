...
def create_sequences(df, labels, sequence_length):
    sequences, y_labels = [], []
    df['is_fraud'] = labels.values  # Add target back for grouping
    grouped = df.groupby(df.index)
    for _, group in grouped:
        group = group.sort_values(by=df.columns[0])  # Sort by first principal component
        values = group.drop(columns=['is_fraud']).values
        targets = group['is_fraud'].values
        n = len(group)
        if n <= sequence_length:
            pad = np.repeat(values[0:1, :], sequence_length - n, axis=0)
            sequences.append(np.concatenate((pad, values), axis=0))
            y_labels.append(targets[-1])
        else:
            for i in range(n - sequence_length):
                sequences.append(values[i:i + sequence_length])
                y_labels.append(targets[i + sequence_length])
    return np.array(sequences), np.array(y_labels)

X_train_seq, y_train_seq = create_sequences(X_train_pca, y_train, sequence_length)
X_test_seq, y_test_seq = create_sequences(X_test_pca, y_test, sequence_length)
